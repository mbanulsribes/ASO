...
<main id="main" tabindex="-1" class="${home?'home-main':'lesson-main'}"><div class="reading"><header class="page-header"><div class="eyebrow">${home?'APRENDER ADMINISTRANDO':p.number?'UNIDAD '+escape(p.number):'EL CURSO'}</div><h1>${escape(p.title)}</h1>${p.summary?`<p class="lead">${escape(p.summary)}</p>`:''}${p.number?`<div class="lesson-meta"><span>${escape(p.period)}</span><span>${escape(p.ra)}</span><a href="fuentes/${p.file}" download>Descargar AsciiDoc ↓</a></div>`:''}</header>${home?milestones:''}<article class="document">${p.doc.convert()}</article>${cards}${pagination}<footer class="page-footer"><span>ASO · IES Font</span><a href="creditos.html">Fuentes y créditos</a></footer></div>${!home?`<aside class="page-toc" aria-label="En esta página"><p>EN ESTA PÁGINA</p>${toc}<div class="toc-note"><strong>Aprender, comprobar, explicar.</strong><span>Cada práctica incluye una alternativa sin acceso al servidor.</span></div></aside>`:''}</main></div></body></html>`;
